...
if [[ -n "${SIGN_IDENTITY:-}" ]]; then
  IDENTITY=$(security find-identity -v -p codesigning 2>/dev/null | grep -F "$SIGN_IDENTITY" | head -1 | awk -F'"' '{print $2}')
else
  IDENTITY=$(security find-identity -v -p codesigning 2>/dev/null | grep "Apple Development" | head -1 | awk -F'"' '{print $2}')
fi
if [[ -z "$IDENTITY" ]]; then
